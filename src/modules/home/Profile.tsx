...
const MyProfile = () => {
  ...
  const navigation = useNavigation<any>();
  const {UserEmail} = useSelector((store: any) => store.AuthenticateReducer);
  const checkLogOut = async () => {
    console.log('USerEmail at User Profile : : >>>>.', UserEmail);
    await dispatch({type: 'User_Data', payload: ''});
    const email: any = await AsyncStorage.removeItem('EMAIL');
    console.log('email at logout', email);
    navigation.navigate('UserLogin');
  };

  const navToScreen = (item: any, index: any) => {
    if (index === 0) {
      return navigation.navigate(routesNames.Order);
    } else if (index === 1) {
      return navigation.navigate(routesNames.HomeScreen);
    }
  };
  return (
    <View style={styles.mainConatiner}>
      ...
      {DataList.map((item: any, index: any) => {
        return (
          <TouchableOpacity
            onPress={() => navToScreen(item, index)}
            key={item?.id}
            activeOpacity={0.7}
